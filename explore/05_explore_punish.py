...
def test_endpoints():
    ...
    logger.info("Token obtained. Probing specific paths discovered from JS...")
    ...
    endpoints = [
        ...
        "ic-web/auth/webapp",
        "ic-web/reserve/punishInfo"
    ]
    
    results = {}
    for ep in endpoints:
        url = HttpClient.get_lib_url(f"{ep}?vpn-12-libseat.njfu.edu.cn")
        res = session.get(url, headers=api_headers, timeout=10)
        logger.info(f"[{ep}] Status: {res.status_code}")
        if res.status_code == 200:
            try:
                data = res.json()
                results[ep] = data
            except:
                results[ep] = res.text[:500]

    with open("c:\\code\\fuck_njfu_lib\\explore\\punish_and_js_api_results.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    logger.info("Done building report for newly discovered endpoints.")
