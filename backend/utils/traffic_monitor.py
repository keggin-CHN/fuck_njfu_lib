...
class LibraryTrafficMonitor:
    ...
    def get_current_traffic():
        ...
        try:
            ...
            if len(all_spans) < 2:
                logger.error(f"流量监控：页面结构解析失败，未找到足够的数据。页面内容: {response.text[:500]}")
                return None

            # 第一个span是剩余座位，第二个span是总座位数
            remaining_text = all_spans[0].text.strip()
            total_text = all_spans[1].text.strip()
            logger.debug(f"流量监控：解析到剩余座位文本: '{remaining_text}', 总座位文本: '{total_text}'")

            remaining_seats = int(remaining_text)
            total_seats = int(total_text)
