...
def add_scheduler_jobs():
    ...
    scheduler.add_job(
        func=keep_alive_all_users,
        trigger=CronTrigger(minute="*/5"),
        id="keep_alive_all_users",
        name="Keep alive sessions for all users every 5 minutes",
        replace_existing=True,
        coalesce=True,
        max_instances=1
    )
...
def keep_alive_all_users():
    ...
    for user in users:
        try:
            ...
            logger.debug(f"用户 {user.username} 保活异常: {e}")
@with_app_context
def auth_all_users():
    logger.info("开始为所有自动预约用户检查/维护认证")
    users = get_users_with_setting(auto_reserve=True)
    for user in users:
        auth = AuthManager.get_authenticator(user)
        if auth and auth.is_valid():
            auth.keep_alive()
            logger.info(f"用户 {user.username} 凭据有效并已发送心跳保活")
        else:
            logger.warning(f"用户 {user.username} 凭据失效，尝试重新认证")
            AuthManager.clear_authenticator(user.id)
            auth = AuthManager.get_authenticator(user)
            if auth:
                logger.info(f"用户 {user.username} 重新认证成功")
            else:
                logger.error(f"用户 {user.username} 认证失败")
    logger.info(f"已完成所有 {len(users)} 个用户的认证检查")
@with_app_context
@handle_exception
