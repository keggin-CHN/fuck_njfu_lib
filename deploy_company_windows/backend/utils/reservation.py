...
class SeatReservation:
    ...
    def ensure_authenticated(self):
        ...
        return bool(self.authenticator and self.authenticator.token and self.authenticator.acc_no)
    ...
    def reserve_seat(self, area, seat_number, seat_id, date_str=None, start_time=None, is_late_protection=False, is_auto_find=False, send_notification=True):
        ...
        try:
            ...
            duration = (end_time_obj - begin_time_obj).total_seconds() / 3600
            if duration < 2:
        ...
        for retry in range(max_retries):
            try:
                ...
                if response is not None and 300 <= response.status_code < 400:
                    message = f"预约接口发生重定向（HTTP {response.status_code}），请检查校内网络或重新登录"
                    logger.error(message)
                    self._record_reservation_history(
                        area, seat_number, seat_id, date_str, start_time, end_time, "失败", message,
                ...
                if response and response.status_code == 200:
                    ...
                    if result.get("code") == 0:
                        ...
                        auth_keywords = ("登录", "Token", "认证")
                        if any(k in error_msg for k in auth_keywords) and retry == 0:
                            logger.warning(f"用户 {self.user.username} 预约时遇到认证错误: {error_msg}，尝试刷新认证后重试")
                            new_auth = AuthManager.refresh_authenticator(self.user)
                            if new_auth and new_auth.token and new_auth.acc_no:
                                self.authenticator = new_auth
                                api_headers["token"] = self.authenticator.token
                                payload["appAccNo"] = self.authenticator.acc_no
                                payload["resvMember"] = [self.authenticator.acc_no]
                                continue
                        
                        no_retry_keywords = ["已被预约", "已有预约", "已预约", "正在被预约", "时长不足", "时间段", "座位号无效", "配置无效", "已经"]
                        if any(k in error_msg for k in (*no_retry_keywords, *auth_keywords)):
