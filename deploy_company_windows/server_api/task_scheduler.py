...
try:
    import config as server_config_module
    Config = server_config_module.Config
except Exception:
    from config import Config

logger = logging.getLogger(__name__)

TASKS_DIR = getattr(Config, "TASKS_DIR", os.path.join(os.path.dirname(os.path.abspath(__file__)), "tasks"))
...
def list_all_tasks() -> list[dict]:
    """列出所有任务。"""
    tasks = []
    for f in Path(TASKS_DIR).glob("*.json"):
        try:
...
_last_keep_alive_time = 0


def _scheduler_loop():
    """后台循环，每 30 秒检查一次。"""
    global _scheduler_running, _last_keep_alive_time
